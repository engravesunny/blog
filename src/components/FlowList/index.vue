<template>
  <div
    ...
    ref="listRef"
    :style="{ height: listInstance.maxHeight.value + 'px' }"
  >
    <div
      class="flow-list-item"
      ref="itemRef"
      :style="{
        width: width ? width + 'px' : itemWidth + '%',
    ...
    >
      <slot
        ...
        :handleLoad="() => handleLoad(item, itemRef[index])"
        name="item"
      >
      </slot>
      <template v-if="!$slots.item">
        <img
          :src="item.imgUrl"
          @load="handleLoad(item, itemRef[index])"
          :alt="item.imgUrl"
        />
      </template>
    </div>
...
const handleLoadMore = (list: ImgItem[]) => {
  loading.value = true;
  // handle load more
  listInstance.curRenderListInfo.value = [
    ...listInstance.curRenderListInfo.value,
    ...list,
  ];
  loading.value = false;
