  ...
  <div class="mid_card" ref="dom">
    <div class="left" @click="toArticle()">
      <img
        v-if="!loading"
        ref="imgDom"
        :src="`${baseURL}/image/loading.gif`"
        :data-src="
          coverUrl ||
          `${postImgUrl}/${postInfo.postImg.split('.')[0] + 'min.webp'}`
        "
        alt="postImg"
      />
      <link v-if="postInfo.post_url" rel="prefetch" :href="postInfo.post_url" />
      <link v-if="postInfo.cover_url" rel="prefetch" :href="preUrl" />
    </div>
    <div class="main">
    ...
    <div class="right">
      ...
      <div class="bottom">
        <div class="date">
          <span class="icon iconfont">&#xe662;</span> {{ postInfo.date }}
        </div>
      </div>
    </div>
  </div>
...
const preUrl = "https://www.kecat.top/" + postInfo.cover_url;
const coverUrl = computed(() => {
  if (postInfo.cover_url) {
    return "https://" + getImageUrl(60, postInfo.cover_url);
  } else {
    return "";
  }
});
const init = async () => {
  ...
  postInfo.post_url = info.post_url;
  postInfo.cover_url = info.cover_url;
  loading.value = false;
};
