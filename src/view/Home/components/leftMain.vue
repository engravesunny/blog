<template>
    <div class="leftMain">
        <!-- 置顶文章 -->
        <big-card v-if="curPage === 0" v-for="(post, index) in topList" :post-img="post.postImg"
            :right="!!(index % 2) || !!(topList.length % 2)" :key="post.name" :isTop="true" :category="post.category"
            :tags="post.tag" :date="post.date" :title="post.name"></big-card>
        <!-- 置顶文章 -->
...
const curList = computed(() => {
    if (curPage.value === 0) {
        ...
        return restTotalList.value.slice(curPage.value * cueSize, curPage.value * cueSize + cueSize) as PostSingle[];
    } else {
        return restTotalList.value.slice(curPage.value * pageSize, curPage.value * pageSize + pageSize) as PostSingle[];
    }
})
