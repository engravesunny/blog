    ...
    <ul class="main_ul">
      <li class="main_li" v-for="item in controls" :key="item.name">
        <div class="main_li_title iconfont" @click="openMainLi(item)">
          ...
          <span
            style="float: right; font-size: 20px; transition: all 0.3s"
            v-if="item.children"
            :style="{
              transform: `rotate(${
                isOpen.indexOf(item.name) > -1 ? 0 : 90
              }deg)`,
            }"
            class="iconfont"
            >&#xe60c;</span
        ...
        <ul
          class="vice_ul shenglue"
          v-if="item.children && isOpen.indexOf(item.name) > -1"
        >
          <li
            :title="child.name"
            class="vice_li iconfont"
