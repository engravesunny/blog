<template>
    ...
    <div
      ...
      :style="{
        backgroundImage: coverUrl
          ? `url(${coverUrl})`
          : `url(${postImgUrl}/${postImg.split('.')[0] + 'min.webp'})`,
      }"
    >
      <div class="site-info">
        <h1>{{ postName }}</h1>
        <div class="top">
          <div class="bottom">
            <div class="date shenglue iconfont" :title="date">
              &#xe663; {{ date }}
            </div>
            <div class="tagList iconfont">
              <div
                class="tag shenglue iconfont"
                :title="item"
  ...
  <img-display
    @closeImgDisplay="hanldeCloseImgDisplay"
    v-if="showDisplayImg"
    :display-img-src="displayImgSrc"
    :display-img-alt="displayImgAlt"
  ></img-display>
</template>
...
import { getPostFromQiniu } from "@/api/post";
...
const getPostImg = async () => {
  ...
  coverUrl.value = res.cover_url
    ? "https://www.kecat.top/" + res.cover_url
    : "";
  postUrl.value = res.post_url || "";
};
...
const handleRouteChange = (val) => {
  try {
    if (val.path === "/article") {
      ...
      if (postUrl.value) {
        getPostFromQiniu(postUrl.value)
          .then((res) => {
        ...
        getPost(encodeURIComponent(val.query.postName + ".md"))
          .then((res) => {
            const converter = new showdown.Converter();
            const htmlOutput = converter.makeHtml(res.data);
            html.value = htmlOutput;
