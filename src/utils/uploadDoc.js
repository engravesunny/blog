...
const uploadDoc = async (
  ...
  let postImg = postImgFile.name || "post_image";
  const nowTime = file.lastModified;
  const file_name = file.name;
  // 上传文章
  if (dir_path.indexOf("/posts") > -1) {
    // 上传文章
    let postUrl = await uploadFile(file, "post/", file_name);
    await uploadFile(postImgFile, "post_cover/", postImgFile.name);
    let coverUrl = "post_cover/" + postImgFile.name;
    ...
    const { data: category } = await createDir({
      dir_path: `./posts/postVirtual/${removeFileExtension(
        file_name
    ...
    const { data: categoryDir } = await createDir({
      dir_path: `./posts/category/${categoryName}/${removeFileExtension(
        file_name
      )}`,
    });
    ...
    // 5.更新json文件
    const isSuccess = await updateJSON(
      categoryName,
      tagNames,
      postImg,
      Date.now(),
